...
function OutputWindow({outputDetails}) {
    const getOutput = () => {
        ...
        if(statusId === 6){
            ...
            return <pre className="px-2 py-1 font-normal text-xs text-green-500">{atob(outputDetails?.stdout) !== null ? `${atob(outputDetails?.stdout)}` : null}</pre>
        } else if(statusId === 5) {
    ...
    <h1 className="font-bold text-xl big-clip-text color-black mb-2">Output</h1>
    <div className="w-full max-w-full h-56 bg-[#1e293b] rounded-md text-white font-normal text-sm overflow-y-auto">
